...
5. 任一失败即拒绝，并定位**链中最先可判定的违规跳**：严格按跳号递增核验，
   跳内顺序为 签名 → 签发关系 → 收紧 → 时效。因此前序已成立的限制放宽不会被
   后续对象的签名失败掩盖；反之当前序号之前均合法时，单独篡改任一对象仍定位
   该对象的签名失败。
...
3. 复核**首个可判定违规定位**：「前序已签名放宽、后续签名被篡改」必须稳定定位前序
   放宽的跳号/字段/说明，并对照「仅末端篡改→该跳签名失败」「更早跳签名失败优先」；
4. 复核篡改签名 / 改写载荷的拒绝（`BAD_SIGNATURE`）；
5. 复核结构性与数值错误定位（重复键、键序、不安全/越界整数、非有限数等）；
6. 运行相关代码测试（`node --test`）与页面构建检查；
7. 启动临时网关做健康地址 API/HTTP 冒烟——包含上述组合场景、单点签名篡改、
   首跳根公钥不符的真实 HTTP 核验（`GATEWAY_URL` 存在时再冒烟对端）。
